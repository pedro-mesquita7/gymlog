...
- Don't use index as key in SortableContext items (use template_id)
  ...
  <verify>
Run `npx tsc --noEmit` and `npx vitest run` -- no errors. Open dev server, navigate to Settings tab, verify rotation section is visible with create/edit/delete UI.
  </verify>
  <done>
