...
**IMPORTANT:** The existing DeleteConfirmation component at `src/components/DeleteConfirmation.tsx` has this interface:
```typescript
...
The component uses `isOpen` to control visibility (returns null if !isOpen).
...
export function ActiveWorkout({ template, exercises, onFinish, onCancel }: ActiveWorkoutProps) {
  ...
  // ... existing navigation code (goToPrev, goToNext, swipeHandlers) ...
...
**Note:** DeleteConfirmation is always rendered but uses `isOpen={showCancelConfirm}` to control visibility. The component internally returns null when !isOpen.
  </action>
